...
// Images are located in "images/webp/".
const projects = {
    "1104 Breakfast Menu": {
      images: [
        "1104 BM (2).webp",
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateModalImage() {
      ...
      void modalImage.offsetWidth; // Force reflow to restart animation.
    ...
    // Shuffle the Portfolio Grid with a Cube Animation.
    ...
    if (grid) {
      ...
      itemsArray.forEach(item => {
        item.classList.add('cube-animate');
      });
      setTimeout(() => {
        itemsArray.forEach(item => item.classList.remove('cube-animate'));
      }, 100);
    }
